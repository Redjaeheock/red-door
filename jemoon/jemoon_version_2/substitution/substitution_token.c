...
/*   Updated: 2025/01/18 20:01:36 by jemoon           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "../minishell.h"

int	substitution_dollar_sign_to_envval(t_data *meta, char **str)
{
	char	*tmp;
	int		row;
	...
	while (str[row] != NULL)
	{
		tmp = search_n_change_dollar_sign(meta, *str);
		if (tmp == NULL)
		{
...
int	mapping_dollar_sign(t_data *meta, t_list *tokens)
{
	...
	result = substitution_dollar_sign_to_envval(meta, tmp);
