...
/*   Updated: 2025/03/14 16:46:28 by jemoon           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
		...
		int *i, int cmd_size)
{
	...
	while (j < f_list_len)
	{
		string_array[*i] = ft_strdup((*tokens)->f_list[j]);
...
void	get_commads_token(char **string_array, t_list **tokens, \
		int *i, int cmd_size)
{
	string_array[*i] = ft_strdup((*tokens)->token);
	if (string_array[*i] == NULL)
		return ;
	(*i)++;
}
...
void	get_commads_key(char **string_array, t_list **tokens, \
		int *i, int cmd_size)
{
	string_array[*i] = ft_strdup((*tokens)->key);
	if (string_array[*i] == NULL)
		return ;
	(*i)++;
}
		...
		int *i, int redifine_size)
{
	...
	while (j < str_size)
	{
		string_array[*i] = ft_strdup(str[j]);
		(*i)++;
		j++;
	}
